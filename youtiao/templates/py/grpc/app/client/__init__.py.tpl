...
sys.path.append(str(APP_BASE_DIR.joinpath('{{ app_name }}', 'proto')))
...
try:
    ...
    raise KeyError('Missing GRPC host config')
try:
    port = APP_CONFIG['app']['port']
except KeyError:
    raise KeyError('Missing GRPC port config')
cli = Client('{{ app_name }}_grpc_client', host, int(port))
cli.set_stub({{ app_name }}_pb2_grpc.{{ app_name }}Stub)
